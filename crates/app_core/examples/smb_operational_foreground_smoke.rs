...
#[derive(Serialize)]
struct SmbOperationalSmokeReport {
    profile: &'static str,
    result: &'static str,
...
impl SmbOperationalSmokeReport {
    fn new() -> Self {
        Self {
            profile: "windows_smb_operational_foreground",
            result: "blocked",
    ...
    fn record_lifecycle(&mut self, status: &sentinel_contracts::EtwLifecycleStatus) {
        ...
        self.record_degraded_reason(status.degraded_reason.clone());
    ...
    fn record_provider_status(&mut self, status: &NetworkProviderControllerStatus) {
        if let Some(provider) = status.provider(NetworkProviderKind::WindowsSmbOperational) {
            ...
            self.record_degraded_reason(provider.degraded_reason.clone());
        ...
        self.auth_consumed = self
            .auth_consumed
            .max(status.provider_zero.smb_operational_auth_consumed);
        ...
        self.remote_admin_consumed = self
            ...
            .max(status.provider_zero.smb_operational_remote_admin_consumed);
        self.lateral_invocations = self
            .lateral_invocations
            .max(status.provider_zero.smb_operational_lateral_invocations);
        self.lateral_consumed = self
            .lateral_consumed
            .max(status.provider_zero.smb_operational_lateral_consumed);
        self.downstream_facts = self
            .downstream_facts
            .max(status.provider_zero.smb_operational_downstream_facts);
        self.provider_zero_smb_only = status.provider_zero.smb_operational_sensing_only();
    }
    ...
    fn record_latest_batch(
        ...
        self.record_degraded_reason(batch.degraded_reason.clone());
    ...
    fn record_degraded_reason(&mut self, reason: Option<String>) {
        if let Some(reason) = reason {
            let stopped_reason = reason.ends_with("_stopped") || reason.contains("session_stopped");
            if self.provider_degraded_reason.is_none() || !stopped_reason {
                self.provider_degraded_reason = Some(reason);
            }
        }
    }
    ...
    fn mark_final_status(&mut self) {
        ...
        self.blocked_reason = Some(if self.provider_enabled == 0 {
            self.provider_degraded_reason
                .clone()
                .unwrap_or_else(|| "smb_operational_channels_unavailable_or_not_ready".to_string())
        } else if self.raw_events == 0 || self.normalized_batches == 0 {
            "smb_operational_no_recent_allowlisted_events".to_string()
        } else if self.auth_detector_invocations == 0 || self.auth_consumed == 0 {
            "smb_operational_no_auth_capable_events_observed".to_string()
        } else if self.downstream_facts == 0 {
            "smb_operational_downstream_consumption_not_observed".to_string()
        } else if !self.clean_shutdown || self.unjoined_workers != 0 {
            "smb_operational_shutdown_join_not_observed".to_string()
        } else {
            "smb_operational_live_realization_incomplete".to_string()
        });
    }
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    let mut report = SmbOperationalSmokeReport::new();
    ...
    if let Some(initial) = container.smb_operational_sensing_lifecycle_status() {
    ...
    match container.activate_smb_operational_sensing(
        &owner,
        vec!["smb_operational_foreground_smoke_authorization".to_string()],
    ) {
        Ok(status) => {
            report.record_provider_status(&status);
            if let Some(lifecycle) = container.smb_operational_sensing_lifecycle_status() {
    ...
    while Instant::now() < deadline {
        ...
        {
            match container.pump_smb_operational_sensing_live_batches() {
                Ok(pump) => {
        ...
        if let Some(batch) = container.latest_smb_operational_batch() {
    ...
    match container.stop_smb_operational_sensing(
        &owner,
        vec!["smb_operational_foreground_smoke_stop".to_string()],
    ) {
        Ok(status) => {
